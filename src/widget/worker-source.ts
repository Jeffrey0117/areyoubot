// Self-contained PoW worker source as a string. esbuild inlines this into the
// single widget bundle; at runtime the widget turns it into a Blob URL and
// spawns a Worker, so the heavy SHA-256 brute-force never blocks the host page.
//
// This MUST stay byte-compatible with src/widget/sha256.ts + pow-solver.ts
// (which are unit-tested against Node crypto). Keeping it as a literal string
// avoids any bundler transform changing the hashing semantics.
